...
class Model():
    ...
    def build(self, embedding_file=None):
        ...
        def cell():
            lstm = tf.nn.rnn_cell.LSTMCell(self.dim_embedding)
            drop = tf.nn.rnn_cell.DropoutWrapper(lstm, output_keep_prob=self.keep_prob)
            return drop
            
        with tf.variable_scope('rnn'):
            
            r_cell = tf.nn.rnn_cell.MultiRNNCell([cell() for _ in range(self.rnn_layers)])
            self.state_tensor = r_cell.zero_state(self.batch_size, tf.float32)
            outputs_tensor, final_state = tf.nn.dynamic_rnn(r_cell, data,initial_state=self.state_tensor)
